...
class IFLYCHAT_CTRL_iflychat extends OW_ActionController {
    ...
    public function auth() {
      ...
      //  $variable_get = '3';
        ...
        $data = array(
            'uname' => ($uid)?$uname:$this->iflychat_get_current_guest_name(),
            'uid' => ($uid)?(string)OW::getUser()->getId():'0-'.$this->iflychat_get_current_guest_id(),
            'api_key' => $api_key,
            'image_path' => '',
            'isLog' => TRUE,
            'role' => $role,
...
try {
    ...
    $response = $this->iflychat_extended_http_request($uri, $options);
if($response->code != 200) {

    $var = array (
        'name' => ($uid)?$uname:$this->iflychat_get_current_guest_name(),
        'uid' => ($uid)?(string)OW::getUser()->getId():'0-'.$this->iflychat_get_current_guest_id()
    );
...
if(isset($jsonData->_i) && ($jsonData->_i!=$variable_get)) {
    ...
    $config->saveConfig('iflychat', 'setting_vars',json_encode(array_merge($data, $data2)));
        ...
        $json['name'] = ($uid)?$uname:$this->iflychat_get_current_guest_name();
        $json['uid'] = ($uid)?(string)OW::getUser()->getId():'0-'.$this->iflychat_get_current_guest_id();
        $json['up'] = self::iflychat_get_user_pic_url();
        $json['upl'] = self::iflychat_get_user_profile_url();
    ...
    catch(Exception $e)
{
    ...
    $var = array (
        'name' => ($uid)?$uname:self::iflychat_get_current_guest_name(),
        'uid' => ($uid)?(string)OW::getUser()->getId():'0-'.self::iflychat_get_current_guest_id()
    );
